...
export class NotificationService {
  ...
  async add(userId: number, notification: Notification): Promise<ResourceLink> {
    return this.http.post<ResourceLink>(ApiManager.getURL('notifications/user/' + userId), notification).toPromise();
  }

  async delete(id: string): Promise<void>{
    return this.http.delete<void>(ApiManager.getURL('notifications/' + id)).toPromise();
  }
}
